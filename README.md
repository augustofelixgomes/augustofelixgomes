# Olá 👋, sou Augusto Felix Gomes

⚡ **Engenheiro de Energia | Projetista Elétrico | Desenvolvedor em formação**
🇧🇷 Brasil

Sou **Engenheiro de Energia e projetista na área elétrica**, atualmente estudando **programação, desenvolvimento web e automação**.

Tenho interesse em unir **Engenharia + Tecnologia + Programação**, explorando novas formas de aplicar software e automação em projetos e processos de engenharia.

---

## 🌐 Onde me encontrar

* 💼 LinkedIn
* 🐙 GitHub
* 📧 E-mail

---

## 🧠 O que faço

* ⚡ Projetos e aplicações na área de Energia e Elétrica
* 📐 Projetos e desenvolvimento na área elétrica
* 💻 Estudo e desenvolvimento de software
* 🌐 Desenvolvimento Web
* 🤖 Automação de processos
* 📊 Exploração de dados aplicados à Engenharia
* 🔧 Desenvolvimento de pequenos projetos e experimentos
* 📚 Aprendizado contínuo em programação e tecnologia

---

## 🛠️ Tech Stack

### Frontend

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge\&logo=html5\&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge\&logo=css3\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge\&logo=javascript\&logoColor=black)

### Backend

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge\&logo=javascript\&logoColor=black)

> Atualmente estudando conceitos de Backend, APIs e desenvolvimento de aplicações.

### DevOps & Tools

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge\&logo=git\&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge\&logo=github\&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge\&logo=visual-studio-code\&logoColor=white)

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=SEU_USUARIO\&show_icons=true\&theme=tokyonight\&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO\&layout=compact\&theme=tokyonight\&theme=tokyonight\&hide_border=true)

---

## 🚀 Filosofia

> *"Aprender, desenvolver e aplicar tecnologia para transformar desafios de engenharia em soluções práticas."*

---

⭐ Se algum projeto for útil ou interessante, considere deixar uma estrela.

🤝 Sempre aberto a **conexões, aprendizado, colaboração e novas ideias** nas áreas de Engenharia, Tecnologia e Desenvolvimento.
